...
import com.xz.hateoascustom.model.Coffee;
import com.xz.hateoascustom.model.CoffeeOrder;
import com.xz.hateoascustom.model.OrderState;
...
//		
	...
	private Resource<Coffee> addCoffee(Link link) {
		...
		ResponseEntity<Resource<Coffee>> resp = 
				restTemplate.exchange(req, new ParameterizedTypeReference<Resource<Coffee>>() {});
	...
	private void addOrder(Link link, Resource<Coffee> coffee) {
		...
		req = RequestEntity.post(items.getTemplate().expand()).body(
				Collections.singletonMap("_links", coffee.getLink("self")));
	...
	private void queryOrders(Link link) {
		ResponseEntity<String> resp = restTemplate.getForEntity(link.getTemplate().expand(), 
				String.class);
